...
def git_fetch_and_reset(repo_path, default_branch='arm-support',
...
def update_saltbox(saltbox_repo_path, saltbox_playbook_file, verbosity=0):
    ...
    git_fetch_and_reset(saltbox_repo_path, "arm-support")
...
def update_sandbox(sandbox_repo_path, sandbox_playbook_file, verbosity=0):
    ...
    git_fetch_and_reset(sandbox_repo_path, "arm-support")
...
def update_sb(sb_repo_path):
    ...
    git_fetch_and_reset(sb_repo_path, "arm-support")
...
def check_and_update_repo(sb_repo_path):
    ...
    try:
        ...
        # Get the current HEAD hash and the upstream arm-support hash
        head_hash = subprocess.check_output(
            ['git', 'rev-parse', 'HEAD'],
            cwd=sb_repo_path
        ).strip()
        upstream_hash = subprocess.check_output(
            ['git', 'rev-parse', 'arm-support@{upstream}'],
            cwd=sb_repo_path
        ).strip()
